function [FullStackComponent]=LoadAndStitchInParallel(ind,Gfilename,depth_image, Widthx, BscansPerY, Lengthy,TimeRepsToUse,NumPatches)
    %% by Nader A.
    %% LOADING LINEAR DATA
    di=[];di=fopen(Gfilename,'r');
        LinearData_ReorIm=fread(di,'int16=>int16');
    fclose(di); %fclose(di2);
    %% SHAPING MATRIX
        FullStackComponent=single(reshape(LinearData_ReorIm,depth_image, Widthx, BscansPerY, Lengthy));
