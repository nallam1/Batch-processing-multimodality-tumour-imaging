...
if Mask2DandMetsOnly_0_OR_Mask3DOnlyAfter_1_OR_AllAtOnce_2==0 || Mask2DandMetsOnly_0_OR_Mask3DOnlyAfter_1_OR_AllAtOnce_2==2
            ...
            if CorregistrationOptFluIsGood==1
                ...
                if exist(fullfile(SegFolder,'SegmentingFilepath.mat')) %%&& useprevious work
                    ...
                    if optFluFolderForSegmentation~=0
                        save(fullfile(SegFolder,'SegmentingFilepath.mat'),'SegmentingFile','-v7.3');
                    end
                end
    ...
    elseif FoundTumourMask2DButCorrectAlignment==2 % Previousligned brightfield (not to TP0- but Just TPx OCT scan, ==1 is for Coreg to TP0- found and selected
        if isequal(TumourMaskType,'TumourVOI')
            if ~exist(fullfile(SegFolder,'TransverseOptFlu_coregtoOCT_PreTime0-.mat'))%if select alligned coreg TPo- just skips alignment steps (==1)
                %% 4) Multi-modal co-registration (opt-flu image selected to OCTA)
                CorregistrationOptFluIsGood=0;%regardless of previous mask existing to assesss whether happy with previous mask or not
...
if glass_1_Tissue_2_both_3_contour==2||glass_1_Tissue_2_both_3_contour==3
    ...
    elseif OnlyTissueLabelTimepoint0==0
        ...
        if exist(PotentialFile1,'file')||(exist(PotentialFile2,'file') && contains(MouseName,DirectoriesBareSkinMiceKeyword))%-- at least made contour with top defined
            if (Mask2DandMetsOnly_0_OR_Mask3DOnlyAfter_1_OR_AllAtOnce_2==1 || Mask2DandMetsOnly_0_OR_Mask3DOnlyAfter_1_OR_AllAtOnce_2==2)
                if exist(PotentialFile1,'file')%load(
                    Mask3DTissueTemp=matfile(PotentialFile1)
                    Mask3DTissueVarname=whos('-file',PotentialFile1);
...
if Mask2DandMetsOnly_0_OR_Mask3DOnlyAfter_1_OR_AllAtOnce_2==1 || Mask2DandMetsOnly_0_OR_Mask3DOnlyAfter_1_OR_AllAtOnce_2==2
    if glass_1_Tissue_2_both_3_contour==1||glass_1_Tissue_2_both_3_contour==3
        ...
        save(SaveFilenameDataCylindricalProj_UnCoregT0{1},'mask_3D_CylindricalProjection_UnCoregT0','-mat','-v7.3');
        save(SaveFilenameDataUnlimited_UnCoregT0{1},'mask_3D_Unrestricted_UnCoregT0','-mat','-v7.3');
        clearvars tumor_maskProj3DRot mask_3D_Unrestricted_UnCoregT0 
    if glass_1_Tissue_2_both_3_contour~=1
        clearvars mask_3D_CylindricalProjection_UnCoregT0
    end
        %                    if glass_1_Tissue_2_both_3_contour==1
        ...
        ylim=DimsVesselsRaw3D(3)%size(StOCT_UnCoregT0_RotatedShifted,3);
        xlim=DimsVesselsRaw3D(2)%size(StOCT_UnCoregT0_RotatedShifted,2);
        zlim=DimsVesselsRaw3D(1)%size(StOCT_UnCoregT0_RotatedShifted,1);
        if BinVesselsAlreadyPrepared==1
    ...
    if glass_1_Tissue_2_both_3_contour==2||glass_1_Tissue_2_both_3_contour==3 %no flattening of tissue (distort vessel shape similar to non-similarity affine transform
        %% Tissue mask application if applicable (no co-registration in depth or laterally to TP0-)
        %% Loading files
        if OnlyTissueLabelTimepoint0==0
            ...
            if glass_1_Tissue_2_both_3_contour==3 && ActiveMemoryOffload==1%Multiple files processes
                clearvars RawStructtemp RawVesstemp BinVesstemp mask_3DTissue BinOCTA_UnCoregT0_TissueMaskApplied RawOCTA_UnCoregT0_TissueMaskApplied StOCT_UnCoregT0_TissueMaskApplied%StOCT_UnCoregT0_TissueMaskApplied RawOCTA_UnCoregT0_TissueMaskApplied BinOCTA_UnCoregT0_TissueMaskApplied
            end
    ...
    if glass_1_Tissue_2_both_3_contour==3 %Full lateral range tissue mask rotated shifted to remove glass
        if exist(fullfile(MaskCreationDraft{1},'mask3D_GlassExc_uncoregTime0-.mat'))
            if OnlyTissueLabelTimepoint0==1
            ...
            elseif OnlyTissueLabelTimepoint0==0 %If permitting drawing contour for every timepoint
...
% if glass_1_Tissue_2_both_3_contour~=1 
    %% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %% To check: efficient to coregister to TP0- if not also removing air-tissue gap?--How it currently is if go through v16 of code Jan 20 2022
    %% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    if Mask2DandMetsOnly_0_OR_Mask3DOnlyAfter_1_OR_AllAtOnce_2==0 || Mask2DandMetsOnly_0_OR_Mask3DOnlyAfter_1_OR_AllAtOnce_2==2
        ...
        load(fullfile(OptFluSegmentationFolder,'TumourMask2D_aligned.mat')); %previously if select alligned coreg TPo- just skips alignment steps above
        %end
...
%if glass_1_Tissue_2_both_3_contour~=1 
    if Mask2DandMetsOnly_0_OR_Mask3DOnlyAfter_1_OR_AllAtOnce_2==1 || Mask2DandMetsOnly_0_OR_Mask3DOnlyAfter_1_OR_AllAtOnce_2==2
        ...
        %% Glass mode for TPx
        if glass_1_Tissue_2_both_3_contour==3%glass_1_Tissue_2_both_3_contour==1 ||
            %                                     if coregRawDataRequested where unecessary
            ...
            % %                                         RawStructVarname=whos('-file',fullfile(saveFolder{1},'RawStruct_UnCoregTime0-_RotShift.mat'));%Never necessary
            %                                         RawVessVarname=whos('-file',fullfile(saveFolder{1},'RawVess_UnCoregTime0-_RotShift.mat'));
            %                                         if BinVesselsAlreadyPrepared==1
            %                                             BinVessVarname=whos('-file',fullfile(saveFolder{1},'BinVess_UnCoregTime0-_RotShift.mat'));
            %                                         end
...
% %                     end
%                 mask_3D_Unrestricted_FromTopContour=mask_3DGlass & tumor_maskProj3DRot;%logical(imwarp(imwarp(mask_3DGlass,transform3DLateral_OCT_Time0Pre{1}),transform3DLateral_OCT_Time0Pre{2},'OutputView',R3Dfixed,'interp','cubic')) & tumor_maskProj3DRot;%mask_3D_Top & %flip(mask_3D_Top & tumor_maskProj3DRot,3);
%                 mask_3D_CylindricalProjection=tumor_maskProj3DRot;
%             end
            %                                 end
        end
        %% Tissue mode for TPx (only coregister laterally)
        if glass_1_Tissue_2_both_3_contour==2 %no flattening of tissue (distort vessel shape similar to non-similarity affine transform
            if OnlyTissueLabelTimepoint0==0 %if drawing it manually for each timepoint--worth saving tissue mask
                ...
                RawOCTA_CoregT0_TissueMaskApplied=double(imwarp(imwarp(RawVesstemp.RawOCTA_UnCoregT0_TissueMaskApplied,transform3DLateral_OCT_Time0Pre{1}),transform3DLateral_OCT_Time0Pre{2},'OutputView',R3Dfixed,'interp','cubic'));%Rout3);
                ...
                %Never necessary
                %                                                 StOCT_CoregT0_TissueMaskApplied =double(imwarp(imwarp(RawStructtemp.StOCT_UnCoregT0_TissueMaskApplied,transform3DLateral_OCT_Time0Pre{1}),transform3DLateral_OCT_Time0Pre{2},'OutputView',R3Dfixed,'interp','cubic'));%Rout3);
                ...
                if BinVesselsAlreadyPrepared==1
                    BinOCTA_CoregT0_TissueMaskApplied=double(imwarp(imwarp(BinVesstemp.BinOCTA_UnCoregT0_TissueMaskApplied,transform3DLateral_OCT_Time0Pre{1}),transform3DLateral_OCT_Time0Pre{2},'OutputView',R3Dfixed,'interp','cubic'));%Rout3);
                ...
                % Mask Fully created no glass and rotated. Perform analysis to fixed depth from glass bottom
                %                                              tumor_maskProj3DRot=shiftdim(repmat(TumourMask2D_aligned,[1,1,DimsVesselsRaw3D(1)]),2);
                %                                             mask_3D_Unrestricted=logical(mask_3DTissue)%,transform3DLateral_OCT_Time0Pre{1}),transform3DLateral_OCT_Time0Pre{2},'OutputView',R3Dfixed,'interp','cubic')) & tumor_maskProj3DRot;%mask_3D_Top & %flip(mask_3D_Top & tumor_maskProj3DRot,3);
                %                                             mask_3D_CylindricalProjection=tumor_maskProj3DRot;
                tumor_maskProj3DRot=shiftdim(repmat(TumourMask2D_aligned_coregTime0,[1,1,DimsVesselsRaw3D(1)]),2);
                mask_3D_Unrestricted_FromTopContour=logical(imwarp(imwarp(mask_3DTissue,transform3DLateral_OCT_Time0Pre{1}),transform3DLateral_OCT_Time0Pre{2},'OutputView',R3Dfixed,'interp','cubic')) & tumor_maskProj3DRot;%mask_3D_Top & %flip(mask_3D_Top & tumor_maskProj3DRot,3);
                mask_3D_CylindricalProjection=tumor_maskProj3DRot;
                % else%if glass_1_Tissue_2_both_3_contour==2 %not worth doing if based on TP0-
            end
            %% Tissue + glass mode for TPx (coregister laterally and in depth since glass removed)
        elseif glass_1_Tissue_2_both_3_contour==3
            if ActiveMemoryOffload==1
                ...
                %                                             RawStructVarname=whos('-file',fullfile(saveFolder{1},'RawStruct_CoregTime0-_RotShift.mat')); %Never necessary
                RawVessVarname=whos('-file',fullfile(saveFolder{1},'RawVess_CoregTime0-_RotShift.mat'));
                if BinVesselsAlreadyPrepared==1
                    BinVessVarname=whos('-file',fullfile(saveFolder{1},'BinVess_CoregTime0-_RotShift.mat'));
                end
            ...
            if OnlyTissueLabelTimepoint0==1
                ...
                % Mask Fully created no glass and rotated. Perform analysis to fixed depth from glass bottom
                tumor_maskProj3DRot=shiftdim(repmat(TumourMask2D_aligned_coregTime0,[1,1,DimsVesselsRaw3D(1)]),2);
                mask_3D_Unrestricted_FromTopContour=logical(tissueMaskTemp.(tissueMaskVarname.name)) & tumor_maskProj3DRot;%;%mask_3DTissue_FromT0_CoregT0%mask_3D_Top & %flip(mask_3D_Top & tumor_maskProj3DRot,3);
                mask_3D_CylindricalProjection=tumor_maskProj3DRot;
            elseif OnlyTissueLabelTimepoint0==0%Manually drawn tissue mask for each TP
                ...
                % Mask Fully created no glass and rotated. Perform analysis to fixed depth from glass bottom
                tumor_maskProj3DRot=shiftdim(repmat(TumourMask2D_aligned_coregTime0,[1,1,DimsVesselsRaw3D(1)]),2);
                mask_3D_Unrestricted_FromTopContour=logical(tissueMaskTemp.(tissueMaskVarname.name)) & tumor_maskProj3DRot;%%mask_3DTissue_CurrentTP_CoregT0%mask_3D_Top & %flip(mask_3D_Top & tumor_maskProj3DRot,3);
                mask_3D_CylindricalProjection=tumor_maskProj3DRot;
            end
        end
    end
elseif (-1<DaysPrecise && DaysPrecise<=0) %If it is timepoint 0- no temporal 3D co-registration affine transforms
    if Mask2DandMetsOnly_0_OR_Mask3DOnlyAfter_1_OR_AllAtOnce_2==0 || Mask2DandMetsOnly_0_OR_Mask3DOnlyAfter_1_OR_AllAtOnce_2==2
        load(fullfile(OptFluSegmentationFolder,'TumourMask2D_aligned.mat'));%BatchOfFolders{countBatchFolder,4})%
        TumourMask2D_aligned_coregTime0=TumourMask2D_aligned;
        save(fullfile(OptFluSegmentationFolder,['TumourMask2D_aligned_coregTime0.mat']),'TumourMask2D_aligned_coregTime0','-v7.3')
    end
    BatchOfFolders{countBatchFolder,4}=fullfile(OptFluSegmentationFolder,'TumourMask2D_aligned_coregTime0.mat');
    if Mask2DandMetsOnly_0_OR_Mask3DOnlyAfter_1_OR_AllAtOnce_2==1 || Mask2DandMetsOnly_0_OR_Mask3DOnlyAfter_1_OR_AllAtOnce_2==2
...
% %                     tumor_maskProj3DRot=shiftdim(repmat(TumourMask2D_aligned_coregTime0,[1,1,DimsVesselsRaw3D(1)]),2);
% %                     end
%                 mask_3D_Unrestricted_FromTopContour=logical(mask_3DGlass) & tumor_maskProj3DRot;%mask_3D_Top & %flip(mask_3D_Top & tumor_maskProj3DRot,3);
%                 mask_3D_CylindricalProjection=tumor_maskProj3DRot;
%             end
        end
        %% Tissue mode for TP0-
        if glass_1_Tissue_2_both_3_contour==2 %no flattening of tissue (distort vessel shape similar to non-similarity affine transform
            if OnlyTissueLabelTimepoint0==0 %if drawing it manually for each timepoint--worth saving tissue mask
                ...
                % Mask Fully created no glass and rotated. Perform analysis to fixed depth from glass bottom
                tumor_maskProj3DRot=shiftdim(repmat(TumourMask2D_aligned_coregTime0,[1,1,DimsVesselsRaw3D(1)]),2);
                mask_3D_Unrestricted_FromTopContour=logical(mask_3DTissue) & tumor_maskProj3DRot;%mask_3D_Top & %flip(mask_3D_Top & tumor_maskProj3DRot,3);
                mask_3D_CylindricalProjection=tumor_maskProj3DRot;
                % else%if glass_1_Tissue_2_both_3_contour==2 %not worth doing if based on TP0-
            end
        ...
        elseif glass_1_Tissue_2_both_3_contour==3
            ...
            if OnlyTissueLabelTimepoint0==1
                ...
                %To Co-registered stOCT (from glass
                %step above) %Never necessary
            ...
            elseif OnlyTissueLabelTimepoint0==0%Manually drawn tissue mask for each TP
                ...
                %To Co-registered stOCT (from glass step above)%Never necessary
                %                                         if ~exist(fullfile(saveFolder{2},'RawStruct_CoregTime0-_isT0_TissueMaskApp.mat'),'file')
                ...
                % Mask Fully created no glass and rotated. Perform analysis to fixed depth from glass bottom
                tumor_maskProj3DRot=shiftdim(repmat(TumourMask2D_aligned_coregTime0,[1,1,DimsVesselsRaw3D(1)]),2);
                mask_3D_Unrestricted_FromTopContour=logical(mask_3DTissue_CurrentTP_CoregT0) & tumor_maskProj3DRot;%mask_3D_Top & %flip(mask_3D_Top & tumor_maskProj3DRot,3);
                mask_3D_CylindricalProjection=tumor_maskProj3DRot;
            end
        end
    end
...
if Mask2DandMetsOnly_0_OR_Mask3DOnlyAfter_1_OR_AllAtOnce_2==1 || Mask2DandMetsOnly_0_OR_Mask3DOnlyAfter_1_OR_AllAtOnce_2==2
    ...
    if VisualizeResults==1
        ...
        if glass_1_Tissue_2_both_3_contour==1
            if BinVesselsAlreadyPrepared==1
                ...
                RawOCTATemp=matfile(fullfile(saveFolder{1},'RawVess_UnCoregTime0-_RotShift.mat'));%svOCTBin=matfile(BatchOfFolders{countBatchFolder,1});
                RawOCTAVarname=whos('-file',fullfile(saveFolder{1},'RawVess_UnCoregTime0-_RotShift.mat'));
        ...
        while isequal(RightOrientation,'n')%=='n'
            ...
            if BinVesselsAlreadyPrepared==1
                if glass_1_Tissue_2_both_3_contour==1
                    nexttile
                    h1=imshowpair(squeeze(sum(mask_3D_CylindricalProjection_UnCoregT0,2)),squeeze(sum(BinOCTA,2)));
                    title('Sagital (yz) plane')
                    nexttile
                    h2=imshowpair(squeeze(sum(mask_3D_CylindricalProjection_UnCoregT0,3)),squeeze(sum(BinOCTA,3)));
                    title('Coronal (xz) plane')
                    nexttile
                    h3=imshowpair(squeeze(sum(mask_3D_CylindricalProjection_UnCoregT0,1)),squeeze(sum(BinOCTA,1)));
                    title('Transverse (xy) plane')
                    
                    title(t,{[MouseTimepoint]; ['Overlay of binarized vessels and tumour mask']})
                    f.WindowState='maximize';
                    figure, imshow3D(cat(2,mask_3D_CylindricalProjection_UnCoregT0-BinOCTA,BinOCTA))
                else
                    nexttile
                    h1=imshowpair(squeeze(sum(mask_3D_Unrestricted_FromTopContour,2)),squeeze(sum(BinOCTA,2)));
                    title('Sagital (yz) plane')
                    nexttile
                    h2=imshowpair(squeeze(sum(mask_3D_Unrestricted_FromTopContour,3)),squeeze(sum(BinOCTA,3)));
                    title('Coronal (xz) plane')
                    nexttile
                    h3=imshowpair(squeeze(sum(mask_3D_Unrestricted_FromTopContour,1)),squeeze(sum(BinOCTA,1)));
                    title('Transverse (xy) plane')
                    
                    title(t,{[MouseTimepoint]; ['Overlay of binarized vessels and tumour mask']})
                    f.WindowState='maximize';
                    figure, imshow3D(cat(2,mask_3D_Unrestricted_FromTopContour-BinOCTA,BinOCTA))
                end
            else
                if glass_1_Tissue_2_both_3_contour==1
                    nexttile
                    h1=imshowpair(squeeze(sum(mask_3D_CylindricalProjection_UnCoregT0,2)),squeeze(sum(RawOCTA,2)));
                    title('Sagital (yz) plane')
                    nexttile
                    h2=imshowpair(squeeze(sum(mask_3D_CylindricalProjection_UnCoregT0,3)),squeeze(sum(RawOCTA,3)));
                    title('Coronal (xz) plane')
                    nexttile
                    h3=imshowpair(squeeze(sum(mask_3D_CylindricalProjection_UnCoregT0,1)),squeeze(sum(RawOCTA,1)));
                    title('Transverse (xy) plane')
                    
                    title(t,{[MouseTimepoint]; ['Overlay of binarized vessels and tumour mask']})
                    f.WindowState='maximize';
                    figure, imshow3D(cat(2,mask_3D_CylindricalProjection_UnCoregT0-RawOCTA,RawOCTA))
                else
                    nexttile
                ...
                while isempty(operation) || ~(isinteger(int8(operation)) && (0<=operation) && (8>operation))
                    ...
                    switch operation
                        case 0
                            RightOrientation='Y';
                            if glass_1_Tissue_2_both_3_contour==1
                                fid = fopen(fullfile(saveFolder{1},'CheckedOrientations.txt'), 'wt');
                            elseif glass_1_Tissue_2_both_3_contour==2|| glass_1_Tissue_2_both_3_contour==3
                                fid = fopen(fullfile(saveFolder{2},'CheckedOrientations.txt'), 'wt');
                            end
        ...
        if glass_1_Tissue_2_both_3_contour==1
            saveas(f,fullfile(saveFolder{1},['FinalMaskCreation.png']))
        elseif glass_1_Tissue_2_both_3_contour==2|| glass_1_Tissue_2_both_3_contour==3
            saveas(f,fullfile(saveFolder{2},['FinalMaskCreation.png']))
        end
    end
    ...
    if glass_1_Tissue_2_both_3_contour==2 ||glass_1_Tissue_2_both_3_contour==3
        if OnlyTissueLabelTimepoint0==0 || (OnlyTissueLabelTimepoint0==1 && glass_1_Tissue_2_both_3_contour==3)
            ...
            %                                                             else--changed
            %                                                             outside
            save(SaveFilenameDataUnlimited{2},'mask_3D_Unrestricted_FromTopContour','-mat','-v7.3');
            save(SaveFilenameDataCylindricalProj{2},'mask_3D_CylindricalProjection','-mat','-v7.3');
            %                                                             end
        end
        %Only saving uncoreg T0 for glass removed 
%     elseif glass_1_Tissue_2_both_3_contour==1 && ~ exist(SaveFilenameDataUnlimited{1},'file') && ~ exist(SaveFilenameDataCylindricalProj{1},'file')%shouldn't even be reaching this code without saving (saved at step 9)
%         save(SaveFilenameDataUnlimited{1},'mask_3D_Unrestricted_FromTopContour','-mat','-v7.3');%variable would have to be loaded around line 596--jan16 2022, but waste of time
