function [coarseContouredI,coarseContouredIGrey, fineContouredI,binaryImageDrawn, xy_drawn]= Coarse_Fine_contour_v8_withLoadingAndAdjustment(answer3,PrefixFLU_BRI,image_for_reference,colourmap, image_to_be_contoured,filenameT,Nautomation,performFineThresholding,CoarseROI_Question,fineContouring_Question,binaryImageDrawn, xy_drawn,OptFluSegmentationFolder)
%% Description
...
if CoarseROI_Question== 3 || CoarseROI_Question== 4 || CoarseROI_Question== 5 %"Load ROI" %%not complete--for loading contour masks if needed
...
if Nautomation==1 || CoarseROI_Question==1  || CoarseROI_Question==3 || CoarseROI_Question==4 || CoarseROI_Question== 5%"Manual""Load ROI"
...
if Nautomation==0 || CoarseROI_Question==2% "No, thanks" %can directly go to fine thresholding
    ...
    if performFineThresholding==1 || fineContouring_Question==1%"On"
        ...
        save(fullfile(TempFileparts{1:3},'2DMasksRepository',[PrefixFLU_BRI, char(filenameT),' ROI mask.mat']),'fineContouredI','-v7.3')%reference image for next contour possibly
    elseif performFineThresholding==0 || fineContouring_Question==0%"Off"
        imageBWT=coarseContouredIGrey;
