function [F_SubStack_Re,F_SubStack_Im,DimsDataFull_pixUsed,DimsDataPatch_pixUsed]=LoadRawDataFullFasterVariableTime7_int8(BatchOfFolders,DataFolderInd,folder1,folder2,files1Cont,files2Cont, DimsDataPatchRaw_pix,DimsDataFull_pix,TimeRepsToUse,Patching,PatchCount)
%%by Nader A.
    ...
    delete(gcp('nocreate'))
        ...
        for ind1=1:length(files1Cont) %join files1Cont with files2Cont and make single 'for' loop for speed?
            if ~contains(files1Cont{ind1},'bg')
                ...
                F_SubStack_Re(ind1)=parfeval(@LoadAndStitchInParallel_Faster_int8,1,ind1,Gfilename1,depth_image, Widthx, BscansPerY, Lengthy,TimeRepsToUse,numStacksConsidered);
            end
            memory
            if ~contains(files2Cont{ind1},'bg')
                ...
                F_SubStack_Im(ind1)=parfeval(@LoadAndStitchInParallel_Faster_int8,1,ind1,Gfilename2,depth_image, Widthx, BscansPerY, Lengthy,TimeRepsToUse,numStacksConsidered);
     ...
     numStacksConsidered=1;%length(files1Cont)-1;
        ...
        for ind1=1:length(files1Cont) %join files1Cont with files2Cont and make single 'for' loop for speed?
            if ~contains(files1Cont{ind1},'bg') && ind1==PatchCount
                ...
                F_SubStack_Re=parfeval(@LoadAndStitchInParallel_Faster_int8,1,ind1,Gfilename1,depth_image, Widthx, BscansPerY, Lengthy,TimeRepsToUse,numStacksPossible);
            end
%             memory
            if ~contains(files2Cont{ind1},'bg') && ind1==PatchCount
                Gfilename2=fullfile(folder2,files2Cont{ind1});%([folder1 'b0_ch1.dat']); % 1st quadrature
                fprintf('Loading Imaginary data, patch %d/%d \n', ind1,numStacksPossible)%                 disp(['Loading Imaginary data, file ', ind]); 
                F_SubStack_Im=parfeval(@LoadAndStitchInParallel_Faster_int8,1,ind1,Gfilename2,depth_image, Widthx, BscansPerY, Lengthy,TimeRepsToUse,numStacksPossible);
            end%SubStack_Re{ind1}=
%             memory
        end
