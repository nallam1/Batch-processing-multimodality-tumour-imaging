function [minBISIM,Alpha1_F,Alpha2_F,iSNRFramey,vessels_processed_binary]=ID_BISIM_Algorithm_iSNRPerVoxel_iSNRrescaled_PerFrame(FrameStructy,FrameD3Dy,NumAlphaVals,Dims,iSNR_size_zVOIpix,BISIM_size_zVOIpix,BISIM_size_xVOIpix,RangeZ_toSample,RangeX_toSample,RangeZ_VOI_window,RangeX_VOI_window,AlphasRangeY)%RangeOfDThresh%Patch_stack_Complex,
    %% by Nader A. 
    %% iSNR computation at frame y
     %Currently based on noise at bottom B-scan (last 5 pixels in depth across full width x of given B-scan-- taking the spatial mean of the temporal variance of all pixels within that bottom of the B-scan region as noise intensity (assuming it is constant at all depths and the mean intensity over time for the signal intensity at every pixel. 
     %It starts by normalizing the intensity of pixels in case oversaturation removal not previously performed. 
     %The iSNR 2D map is simply the ratio of the spatial mean of the temporal
     %variance of the bottom most pixels to the temporal mean of the signal
     %intensity at every pixel. Please note that the noise intensity is based on the variance as we expect the temporal mean to equal zero for white noise, you can try out the root mean square but that might not make a big difference. 
     %try out: 
        %1) Set the noise based again on the spatial mean of the temporal variance across all pixels per B-scan but this time, from the acquired scan in air (without any samples) instead of based on the bottom most pixels in the considered B-scan
        %2) Set the noise calculation as a function of depth: based on background A-scan acquired (bg_....dat) or based on the 3D scan in air without any sample 
        %3) Manually sampling pixels corresponding to air in a few of the B-scans to perform the variance across--> Using [z,x] = ginput(numPointsToSample)
    %The generated iSNR map is then normalized, and the alpha range search
    %is rescalled based on the iSNR histogram (I noticed few pixels have huge iSNR while most have much lower iSNR making the alpha range ineffective for optimization without some rescalling). 
    FrameStructy=rescale(FrameStructy); % if not already rescaled by Oversaturation removal (e.g., if set to 0 os removal), equivalent
          ...
          %iSNRFullFramey=Time_Var_noise./Time_Mean_Signal;
            ...
            %iSNR never high enough to allow alpha to make difference otherwise
            %% Replication Padding for next step of structural vector computation
...
%             fprintf('y=%d, BISIM for multiple alpha_t computation, each at multiple (z,x) coordinates ...\n',y);
            %% Iterating through decorrelation thresholds (alpha_thresh) 
            for alpha_t_ind=1:NumAlphaVals
                %Iterating through decorrelation thresholds based on
                %iSNR versus CDV general distribution, computing structural vectors of
                %resulting binarized vessels map and the BISIM metric
...
%             fprintf('\nDetermining optimal BISIM for current patch at y=%d\n',y)
            ...
            %% Identification of optimal BISIM
