...
%         ProcScriptDirectory='D:\Processing code\OCT\1) SvOCT processing\Codes_for_Ottawa\Main for bulk processing-Nader'
    ...
    %% Data processing settings    
        ...
        OS_removal=0.1;%0.03;%Keep it raw %0.003--empirically found to remove oversaturation from glass but not best
    ...
    %% System specifications
        LamCent=1.32*10^-6;%Central wavelength in um
        LamFWHM=.11*10^-6;%bandwidth (FWHM wavelength) in um

        z_res=(2*log(2)/pi)*(LamCent^2/LamFWHM);% https://www.nature.com/articles/s41598-017-18635-w % calculated 8um not 7um 
        xy_res=15;%from focal length measurements %https://link.springer.com/referenceworkentry/10.1007/978-3-319-06419-2_10
    ...
    %% Physical to pix conversions
            ...
            DataCroppedNotResizedInDepth=0;% was data cropped or is it unaltered from usual 500pix in depth acquired scan and possibly just resized
...
%%%%%%%%%%%%%%%%%%%%%%
    ...
    ErrorPermittedAxially=ceil(z_res)/(500/ReferencePixDepth)
        ...
        temp3=strsplit(temp2{1},'x')
        xy=[str2num(temp3{1}),str2num(temp3{2})];
        xDim_um=min(xy)*1000;  %extract from file name the FOV dimensions.
        ErrorPermittedxLaterally=ceil(ceil(xy_res)/(xDim_um/DimsDataFull_pix(2)));%maximum deviation acceptable before glass considered in a different location axially
        if DataCroppedNotResizedInDepth==1
            ErrorPermittedAxially=ceil(ceil(z_res)/(mean([umPerPix_For200PixDepthInAir,(umPerPix_For200PixDepthInAir/IndxRefGlass)],'all')*(ReferencePixDepth/500)));%maximum deviation acceptable before glass considered in a different location axially
        elseif DataCroppedNotResizedInDepth==0
            ErrorPermittedAxially=ceil(ceil(z_res)/(mean([umPerPix_For200PixDepthInAir,(umPerPix_For200PixDepthInAir/IndxRefGlass)],'all')*(ReferencePixDepth/DimsDataFull_pix(1))));%maximum deviation acceptable before glass considered in a different location axially
        end
        % patch subdivision determination
       mem=memory
