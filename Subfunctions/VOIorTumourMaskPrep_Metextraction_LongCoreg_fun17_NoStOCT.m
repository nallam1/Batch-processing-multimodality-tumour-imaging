        ...
        if FoundTumourMask2DButCorrectAlignment==0 % was this not at all already not?
            if isequal(TumourMaskType,'TumourVOI')
                ...
                elseif isequal(TumourMaskType,'PrismVOI')
                    ...
                    save(fullfile(SegFolder,[PrefixFLU_BRI_Var,'TumourMask2D_aligned.mat']),'TumourMask2D_aligned','-v7.3')%well here no real difference
                end
...
if (Neither_0_glass_1_Tissue_2_both_3_contour==2||Neither_0_glass_1_Tissue_2_both_3_contour==3) && Timepoint0Analyzed==1 %Is the analysis being conducted before timepoint 0- has been acquired and been processed
    ...
    elseif OnlyTissueLabelTimepoint0==0
        ...
        if (~exist(PotentialFile1,'file')||~exist(PotentialFile2,'file'))
            mask_3DTissue=ContouringFunctionFilesLoadedOrMatFile12_RatioDimensionsTis(DimsVesselsRaw3D,num_contoured_slices(2),[MouseName,' ',TimepointTrueAndRel],BatchOfFolders,countBatchFolder, fullfile(MaskCreationDraft{2},'zline_TissueTop_uncoregTime0-.mat'),fullfile(MaskCreationDraft{2},'zline_TissueBot_uncoregTime0-.mat'),fullfile(MaskCreationDraft{2},'mask3D_TissueAndAllBelow_uncoregTime0-.mat'),fullfile(MaskCreationDraft{2},'mask3D_TissueOnly_uncoregTime0-.mat'),saveFolder{2},MaskCreationDraft{2},'Matfile',AutoProcess,OSremoval,Mask2DandMetsOnly_0_OR_Mask3DOnlyAfter_1_OR_AllAtOnce_2);%,umPerPix_For200PixDepth);%             [mask_3D_NoGlass_NotCoreg]= ContouringFunctionFilesLoadedOrMatFile7_batchProc(num_contoured_slices_Top,[MouseName,' ',Timepoint],stOCTLateralCoregistered,vessels_processed_binaryLateralCoregistered,DimsVesselsRaw3D, fullfile(GlassRemovalDraft,'zline_GlassTop.mat'),fullfile(GlassRemovalDraft,'zline_GlassBot.mat'),fullfile(saveFolder,'mask_3D_YesGlassUnrotated.mat'),fullfile(saveFolder,'mask_3D_NoGlassUnrotated.mat'),saveFolder,GlassRemovalDraft,AutoProcess,OSremoval);
        else
            if (Mask2DandMetsOnly_0_OR_Mask3DOnlyAfter_1_OR_AllAtOnce_2==1 || Mask2DandMetsOnly_0_OR_Mask3DOnlyAfter_1_OR_AllAtOnce_2==2)
                if exist(PotentialFile1,'file')%load(
...
if Mask2DandMetsOnly_0_OR_Mask3DOnlyAfter_1_OR_AllAtOnce_2==1 || Mask2DandMetsOnly_0_OR_Mask3DOnlyAfter_1_OR_AllAtOnce_2==2
    if Neither_0_glass_1_Tissue_2_both_3_contour==1||Neither_0_glass_1_Tissue_2_both_3_contour==3
        ...
        %% Loading files
        %Remove all options for DAY 0 (since no organization of folders???)
        ...
        ylim=size(RawOCTA_UnCoregT0_RotatedShifted,3);
        xlim=size(RawOCTA_UnCoregT0_RotatedShifted,2);
        zlim=size(RawOCTA_UnCoregT0_RotatedShifted,1);
        if BinVesselsAlreadyPrepared==1
        ...
        if Neither_0_glass_1_Tissue_2_both_3_contour==1
            ...
            load(fullfile(OptFluSegmentationFolder,[PrefixFLU_BRI_Var,'TumourMask2D_aligned.mat']))%load(BatchOfFolders{countBatchFolder,4});
            ...
            mask_3D_CylindricalProjection=logical(tumor_maskProj3DRot);
        end
...
%
%                 if DaysPrecise~=0 || isempty(DaysPrecise)% for TPx %if ~(sum(contains(DirectoryInitialTimepoints,NameDayFormat),'all') && contains(NameDayFormat,DirectoriesBareSkinMiceKeyword)) && ~(contains(NameDayFormat,DirectoryInitialTimepoints) && contains(BatchOfFolders{countBatchFolder,1},'pre'))%those with tumours have pre and post timepoints
...
%                                 end
%                        end
%                 elseif DaysPrecise==0 %If it is timepoint 0- no temporal 3D co-registration affine transforms
%                     if Mask2DandMetsOnly_0_OR_Mask3DOnlyAfter_1_OR_AllAtOnce_2==0 || Mask2DandMetsOnly_0_OR_Mask3DOnlyAfter_1_OR_AllAtOnce_2==2
    ...
    if VisualizeResults==1
        ...
        if Neither_0_glass_1_Tissue_2_both_3_contour==1
            if BinVesselsAlreadyPrepared==1
                ...
                RawOCTAVarname=whos('-file',fullfile(saveFolder{1},'BinVess_UnCoregTime0-_RotShift.mat'));
        ...
        elseif Neither_0_glass_1_Tissue_2_both_3_contour==2 && Timepoint0Analyzed==1 %Is the analysis being conducted before timepoint 0- has been acquired and been processed 
            if OnlyTissueLabelTimepoint0==0
                if BinVesselsAlreadyPrepared==1
                    ...
                    BinOCTAVarname=whos('-file',fullfile(saveFolder{2},'BinVess_UnCoregTime0-_NotInZ_TissueMaskApp.mat'));
                    BinOCTA=imresize3(BinOCTATemp.(BinOCTAVarname.name),DimsVesselsRaw3D);
                    clearvars BinOCTATemp
                else
                    RawOCTATemp=matfile(fullfile(saveFolder{2},'RawVess_UnCoregTime0-_NotInZ_TissueMaskApp.mat'));%svOCTBin=matfile(BatchOfFolders{countBatchFolder,1});
                    RawOCTAVarname=whos('-file',fullfile(saveFolder{2},'RawVess_UnCoregTime0-_NotInZ_TissueMaskApp.mat'));
                    RawOCTA=RawOCTATemp.(RawOCTAVarname.name);
                    DimsVessels3D=size(RawOCTA);
                    clearvars RawOCTATemp
                end
        ...
        elseif Neither_0_glass_1_Tissue_2_both_3_contour==3 && Timepoint0Analyzed==1 %Is the analysis being conducted before timepoint 0- has been acquired and been processed 
            if (-1<DaysPrecise && DaysPrecise<=0)
                if BinVesselsAlreadyPrepared==1
                    ...
                    BinOCTAVarname=whos('-file',fullfile(saveFolder{2},'BinVess_UnCoregTime0-_isT0_TissueMaskApp.mat'));
                    BinOCTA=imresize3(BinOCTATemp.(BinOCTAVarname.name),DimsVesselsRaw3D);
                    clearvars BinOCTATemp
                else
                    RawOCTATemp=matfile(fullfile(saveFolder{2},'RawVess_UnCoregTime0-_isT0_TissueMaskApp.mat'));%svOCTBin=matfile(BatchOfFolders{countBatchFolder,1});
                    RawOCTAVarname=whos('-file',fullfile(saveFolder{2},'RawVess_UnCoregTime0-_isT0_TissueMaskApp.mat'));
                    RawOCTA=RawOCTATemp.(RawOCTAVarname.name);
                    DimsVessels3D=size(RawOCTA);
                    clearvars RawOCTATemp
                end
                ...
                if OnlyTissueLabelTimepoint0==0
                    if BinVesselsAlreadyPrepared==1
                        BinOCTATemp=matfile(fullfile(saveFolder{2},'BinVess_UnCoregTime0-_CurrentTP_TissueMaskApp.mat'));
                        BinOCTAVarname=whos('-file',fullfile(saveFolder{2},'BinVess_UnCoregTime0-_CurrentTP_TissueMaskApp.mat'));
                        BinOCTA=imresize3(BinOCTATemp.(BinOCTAVarname.name),DimsVesselsRaw3D);
                        clearvars BinOCTATemp
                    else
                        RawOCTATemp=matfile(fullfile(saveFolder{2},'RawVess_UnCoregTime0-_CurrentTP_TissueMaskApp.mat'));%svOCTBin=matfile(BatchOfFolders{countBatchFolder,1});
                        RawOCTAVarname=whos('-file',fullfile(saveFolder{2},'RawVess_UnCoregTime0-_CurrentTP_TissueMaskApp.mat'));
                        RawOCTA=RawOCTATemp.(RawOCTAVarname.name);
                        DimsVessels3D=size(RawOCTA);
                        clearvars RawOCTATemp
                    end
                elseif OnlyTissueLabelTimepoint0==1
                    if BinVesselsAlreadyPrepared==1
                        BinOCTATemp=matfile(fullfile(saveFolder{2},'BinVess_UnCoregTime0-_FromT0_TissueMaskApp.mat'));
                        BinOCTAVarname=whos('-file',fullfile(saveFolder{2},'BinVess_UnCoregTime0-_FromT0_TissueMaskApp.mat'));
                        BinOCTA=imresize3(BinOCTATemp.(BinOCTAVarname.name),DimsVesselsRaw3D);
                        clearvars BinOCTATemp
                    else
                        RawOCTATemp=matfile(fullfile(saveFolder{2},'RawVess_UnCoregTime0-_FromT0_TissueMaskApp.mat'));%svOCTBin=matfile(BatchOfFolders{countBatchFolder,1});
                        RawOCTAVarname=whos('-file',fullfile(saveFolder{2},'RawVess_UnCoregTime0-_FromT0_TissueMaskApp.mat'));
                        RawOCTA=RawOCTATemp.(RawOCTAVarname.name);
                        DimsVessels3D=size(RawOCTA);
                        clearvars RawOCTATemp
                    end
