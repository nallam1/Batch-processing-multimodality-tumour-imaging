...
%                     predrawnTumourMaskFileKeyword=[sprintf('FLU_%s',predrawnTumourMaskFileKeyword(ind))];%%default tumour masks made from fluorescence files%if made from brightfield tumour masks made 
%                 end
            elseif TumourMaskFrom_FLU_0_BRI_1==1
                for ind=1:length(predrawnTumourMaskFileKeyword)
                    predrawnTumourMaskFileKeyword=[sprintf('BRI_%s',predrawnTumourMaskFileKeyword(ind))];%if made from brightfield tumour masks made 
                end
            end
                                ...
                                %% 2D/3D coregistration, Gross tumour response metrics and longitudinal coregistration
                                    ...
                                    if AlreadyCompletedJustContouring==0 || AlreadyCompletedMaskCreation==0
                                        ...
                                        if LateralorFullCoregPreTisContour==0
                                            if UnformatedDataSet==1
                                                BatchOfFolders = VOIorTumourMaskPrep_Metextraction_LongCoreg_fun17_NoStOCT(TumourMaskFrom_FLU_0_BRI_1,GlassThickness_200PixDepth,DataCroppedNotResizedInDepth,ReferencePixDepth,BatchOfFolders,countBatchFolder,MouseName,TimepointTrueAndRel,NameDayFormat,NameTimepointComboTemp,DirectoryInitialTimepoints,DirectoryVesselsData,DirectoryDataLetter,FolderConsidered,DirectoriesBareSkinMiceKeyword,OptFluSegmentationFolder,MaskCreationDraft,saveFolder,pathOCTVesRaw,filenameOCTVesRaw,pathOCTVesBin,filenameOCTVesBin,pathStOCT,filenameStOCT,RawVasculatureFileKeywordRaw,TryAutomaticAllignment,exposureTimes_BriFlu,num_contoured_slices,OSremoval,TumourMaskAndStepsDir,OCTLateralTimepointCoregistrationFolder,FoundTumourMask2DButCorrectAlignment,SaveFilenameDataUnlimited,SaveFilenameDataCylindricalProj,SaveFilenameDataUnlimited_UnCoregT0,SaveFilenameDataCylindricalProj_UnCoregT0,BinVesselsAlreadyPrepared,Mask2DandMetsOnly_0_OR_Mask3DOnlyAfter_1_OR_AllAtOnce_2,VisualizeResults,glass_1_Tissue_2_both_3_contour,DoseReceivedUpToTP,DaysPrecise,OnlyTissueLabelTimepoint0,ActiveMemoryOffload);                                    
                                            else
                                ...
                                FoundMask2D=0;
                                    for SubFolderSearchInd=1:length(OptFluSegmentationFolderFiles) %searching through current folder for the appropriate flu or opt file
                                                        for trialMask2D=1:length(predrawnTumourMaskFileKeyword)
                                                            OptFluSegmentationFolderFiles{SubFolderSearchInd}
                                                            if contains(,predrawnTumourMaskFileKeyword{trialMask2D})
                                                                FoundMask2D=1;
