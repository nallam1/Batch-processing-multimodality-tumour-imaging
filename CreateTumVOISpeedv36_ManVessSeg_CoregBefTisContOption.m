clear
DirectoryProcCodeLetter='D:\git\BatchProcessOptical_OCT-BRI-FLU'%'C:\Users\admin\Desktop'; %'D:'
DirectoryDataLetter='H';%'I:'; %'H:'; %'J:'
DirectoryVesselsData ='H:\SBRT project March-June 2021'%'I:\Test orthotopic mice' %'F:\DLF data' %'H:\Processing DLF Nov 25 2021\Healthy (Bare Skin)'%%[DirectoryVesselLetter '\DLF data']%'G:\DLF data'%'C:\Users\nader\Desktop\DLF data';%'G:\SBRT project March-June 2021'%'G:\PDXovo';
...
addpath(genpath(DirectoryProcCodeLetter))
...
RepeatCheck2D=1;%allow to check 2D brightfield/fluorescence mask just shows last attempt at given timepoint and you accept it or not. if accepted it will just skip all other steps if already completed before.
...
% General
MatlabVer=2021;
Timepoint0Analyzed=0; %Is the analysis not being conducted before timepoint 0- has been acquired and been processed--will have to run a separate code to add times to fluorescence metric extraction data file and and apply co-registration step
...
PredrawnTumourMaskMaybeMisnamed=0;%use pre-drawn transverse mask if exist and not automatically found
...
OSremoval=0.003;%0.003%0.003;%0.003
% Saving info
...
% General
BinarizeVesselsNOW_0_no_1_manual_2_automatic=0;%(0) Do not perform vessel segmentation now (1) Perform manual vessel segmentation after mask creation step, (2) Perform automatic vessel segmentation after mask creation step
...
if (BinarizeVesselsNOW_0_no_1_manual_2_automatic== 1 || BinarizeVesselsNOW_0_no_1_manual_2_automatic== 2) && Neither_0_glass_1_Tissue_2_both_3_contour==0
    error('Cannot have BinVesselsAlreadyPrepared==1 if you will be not be creating a 3D mask.')
end
...
if stOCTAvailability==0 && (Neither_0_glass_1_Tissue_2_both_3_contour==2||Neither_0_glass_1_Tissue_2_both_3_contour==3)
    Neither_0_glass_1_Tissue_2_both_3_contour=1;
...
if TumourMaskFrom_FLU_0_BRI_1_Varies_2==0
    PrefixFLU_BRI='FLU_';
    for ind=1:length(predrawnTumourMaskFileKeyword)
        predrawnTumourMaskFileKeyword{ind}=[sprintf('FLU_%s',predrawnTumourMaskFileKeyword{ind})];%%default tumour masks made from fluorescence files%if made from brightfield tumour masks made
    end
elseif TumourMaskFrom_FLU_0_BRI_1_Varies_2==1
    PrefixFLU_BRI='BRI_';
    for ind=1:length(predrawnTumourMaskFileKeyword)
        predrawnTumourMaskFileKeyword{ind}=[sprintf('BRI_%s',predrawnTumourMaskFileKeyword{ind})];%if made from brightfield tumour masks made
    end
elseif TumourMaskFrom_FLU_0_BRI_1_Varies_2==2
    PrefixFLU_BRI='Var_';
    for ind=1:length(predrawnTumourMaskFileKeyword)
        predrawnTumourMaskFileKeyword{ind}=[sprintf('Var_%s',predrawnTumourMaskFileKeyword{ind})];%if made from brightfield tumour masks made
    end 
end
...
DirectoriesMice={'L0R1';'L1R2';'L1R3';};%{'BSOrt1'}{'BS1';'BS3'};%{'L0R1';'L0R2';'L0R3';'L0R4';'L1R1';'L1R2';'L1R3';'L2R2';'L2R4'};%'\DLF data\20 Gy\M26 - double tumor-(looking at BOTTOM one)';'\DLF data\10 Gy'};%'\DLF data\20 Gy\M44';'\DLF data\20 Gy\M45';'\DLF data\20 Gy\M58';'\DLF data\30 Gy'; '\DLF data\Control'};%
        ...
        if filenameOCTVesRaw==0
            ...
            if UnformatedDataSet==1
                ...
                if Timepoint0Analyzed==1 %Is the analysis being conducted before timepoint 0- has been acquired and been processed
                    ...
                    [CurrentTimepoint,TimeElapsed, MouseName,Timepoint,InitialTimepointtxtFile]=CalcTimePostRTx(TumourMaskAndStepsDir, DirectoryVesselsData);
            ...
            %% 2D mask file to create
            TumourMask2DUncoregT0Filename=fullfile(TumourMaskAndStepsDir,'2D OCT-bri\flu Co-registration + Tumour Mask',[PrefixFLU_BRI,'TumourMask2D_aligned.mat']);
            
            %% 3D mask and OCT files to create
            if Neither_0_glass_1_Tissue_2_both_3_contour==1 || Neither_0_glass_1_Tissue_2_both_3_contour==3
                ...
                SaveFilenameDataCylindricalProj_UnCoregT0{1}=[fullfile(saveFolder{1},[PrefixFLU_BRI,SaveFilenameCylindricalProj '_UnCoregT0.mat'])];
            ...
            if (Neither_0_glass_1_Tissue_2_both_3_contour==2 || Neither_0_glass_1_Tissue_2_both_3_contour==3) && Timepoint0Analyzed==1 %Is the analysis being conducted before timepoint 0- has been acquired and been processed
                ...
                if -1<DaysPrecise && DaysPrecise<=0 %initial timepoint
                    if OnlyTissueLabelTimepoint0==0 %labelling each timepoint tissue
                        ...
                        elseif Neither_0_glass_1_Tissue_2_both_3_contour==3
                    ...
                    elseif OnlyTissueLabelTimepoint0==1
                        if Neither_0_glass_1_Tissue_2_both_3_contour==3
                            SaveFilenameRawMaskApplied{2}=fullfile(saveFolder{2},'RawVess_CoregTime0-_isT0_TissueMaskApp.mat');
                            BinVasculatureFileKeyword={fullfile(saveFolder{2},'Bin Manual','BinVess_CoregTime0-_isT0_TissueMaskApp.mat')};
                        elseif Neither_0_glass_1_Tissue_2_both_3_contour==2
                    ...
                    SaveFilenameDataCylindricalProj_UnCoregT0{2}=[fullfile(saveFolder{2},[PrefixFLU_BRI,SaveFilenameCylindricalProj '_isT0_UnCoregT0.mat'])];
                    SaveFilenameDataUnlimited{2}=[fullfile(saveFolder{2},[SaveFilenameUnrestricted{2} '_isT0.mat'])];
                    SaveFilenameDataCylindricalProj{2}=[fullfile(saveFolder{2},[PrefixFLU_BRI,SaveFilenameCylindricalProj '_isT0.mat'])];
                    ...
                    if OnlyTissueLabelTimepoint0==0 %labelling each timepoint tissue
                        ...
                        SaveFilenameDataCylindricalProj_UnCoregT0{2}=[fullfile(saveFolder{2},[PrefixFLU_BRI,SaveFilenameCylindricalProj '_CurrentTP_UnCoregT0.mat'])];
                        SaveFilenameDataUnlimited{2}=[fullfile(saveFolder{2},[SaveFilenameUnrestricted{2} '_CurrentTP.mat'])];
                        SaveFilenameDataCylindricalProj{2}=[fullfile(saveFolder{2},[PrefixFLU_BRI,SaveFilenameCylindricalProj '_CurrentTP.mat'])];
                        if Neither_0_glass_1_Tissue_2_both_3_contour==2
                            BinVasculatureFileKeyword={fullfile(saveFolder{2},'Bin Manual','BinVess_CoregTime0-_NotInZ_TissueMaskApp.mat')};%BinVasculatureFileKeyword=fullfile(saveFolder{2
                        elseif Neither_0_glass_1_Tissue_2_both_3_contour==3
                    ...
                    elseif OnlyTissueLabelTimepoint0==1
                        ...
                        SaveFilenameDataCylindricalProj_UnCoregT0{2}=[fullfile(saveFolder{2},[PrefixFLU_BRI,SaveFilenameCylindricalProj '_FromT0_UnCoregT0.mat'])];
                        SaveFilenameDataUnlimited{2}=[fullfile(saveFolder{2},[SaveFilenameUnrestricted{2} '_FromT0.mat'])];
                        SaveFilenameDataCylindricalProj{2}=[fullfile(saveFolder{2},[PrefixFLU_BRI,SaveFilenameCylindricalProj '_FromT0.mat'])];
                    end
                end
            end
            ...
            MissingFilesForAutomation= Neither_0_glass_1_Tissue_2_both_3_contour==0 || (Mask2DandMetsOnly_0_OR_Mask3DOnlyAfter_1_OR_AllAtOnce_2==1) && ((Neither_0_glass_1_Tissue_2_both_3_contour==1 && ~exist(saveContour{1})) || (Neither_0_glass_1_Tissue_2_both_3_contour==2 && ~exist(saveContour{2})) || (Neither_0_glass_1_Tissue_2_both_3_contour==3 && ~exist(saveContour{1}) && ~exist(saveContour{2})))
            if MissingFilesForAutomation==1
                ...
                AlreadyCompletedMaskCreation=((Neither_0_glass_1_Tissue_2_both_3_contour==1 && exist(SaveFilenameDataUnlimited_UnCoregT0{1})) || (Neither_0_glass_1_Tissue_2_both_3_contour==2 && exist(SaveFilenameDataUnlimited{2})) || (Neither_0_glass_1_Tissue_2_both_3_contour==3 && exist(SaveFilenameDataUnlimited{1}) && exist(SaveFilenameDataUnlimited{2})))
                ...
                PotentiallyNotWorthProcessing=(AlreadyCompletedTumourMask2D && Neither_0_glass_1_Tissue_2_both_3_contour==0)||(AlreadyCompletedTumourMask2D && AlreadyCompletedJustContouring && Mask2DandMetsOnly_0_OR_Mask3DOnlyAfter_1_OR_AllAtOnce_2==0)|| (AlreadyCompletedTumourMask2D && AlreadyCompletedJustContouring && AlreadyCompletedMaskCreation && (Mask2DandMetsOnly_0_OR_Mask3DOnlyAfter_1_OR_AllAtOnce_2==1 || Mask2DandMetsOnly_0_OR_Mask3DOnlyAfter_1_OR_AllAtOnce_2==2))&& AlreadyCompletedBinarizationIfapplicable;
                StillPursue=[];
                ...
                if (PotentiallyNotWorthProcessing==0 && isempty(StillPursue))|| (PotentiallyNotWorthProcessing==1 && ~(isequal(StillPursue,'n')||isequal(StillPursue,'N')))%elseif~exist(fullfile(saveFolder,'CheckedOrientationsL11.txt'))%elseif ~exist(SaveFilenameDataUnlimited) && ~exist(fullfile(saveFolder,'CheckedOrientations.txt'))
                        ...
                        if Timepoint0Analyzed==1 %Is the analysis being conducted before timepoint 0- has been acquired and been processed
                            ...
                            if ~(-1<DaysPrecise && DaysPrecise<=0)
                                ...
                                try
                                    ...
                                    if Neither_0_glass_1_Tissue_2_both_3_contour==2 || Neither_0_glass_1_Tissue_2_both_3_contour==3
                            ...
                            if Neither_0_glass_1_Tissue_2_both_3_contour==2 || Neither_0_glass_1_Tissue_2_both_3_contour==3
                                timepoint0AlreadyProcQuestion= (sum(contains({timepoint0ProcDir(:).name},'zline_GlassBot_uncoregTime0-.mat'))+sum(contains({timepoint0ProcDir(:).name},'zline_TissueTop_uncoregTime0-.mat')))>1%>0;
                            elseif Neither_0_glass_1_Tissue_2_both_3_contour==1
                    ...
                    if filenameStOCT==0
                        ...
                        if PredrawnTumourMaskMaybeMisnamed==1%if previously drawn can be loaded
                            if exist(fullfile(pathOCTVesRaw,[PrefixFLU_BRI,'TumourFileUsed.mat']),'file')
                                load(fullfile(pathOCTVesRaw,[PrefixFLU_BRI,'TumourFileUsed.mat']))
                                ...
                                if isequal(filename2DMask,0) || isequal(path2DMask,0)%not found this previously drawn mask
                                    ...
                                    save(fullfile(pathOCTVesRaw,[PrefixFLU_BRI,'TumourFileUsed.mat']),'TumourFileUsed','-v7.3');
                                    BatchOfFolders{countBatchFolder,4}=TumourFileUsed;
                                end
                            end
                        else
                        ...
                        if isequal(filename2DMask,[PrefixFLU_BRI,'TumourMask2D_aligned_coregTime0.mat']) %Selecting this only means it skips even checking for alignment step (between TPX OCT and TPX opt flu
                        ...
                        elseif isequal(filename2DMask,[PrefixFLU_BRI,'TumourMask2D_aligned.mat'])
                            createTumourMaskQuestion='y';
                        ...
                        %% Covering situation where Timepoint0- not yet analyzed since possibly not yet acquired or known.
                        if Timepoint0Analyzed==0
                            TimepointTrueAndRel=[];
                            IndexMouse=[];
                            DoseReceivedUpToTP=[];
                            DaysPrecise=[];
                            SegmentationFolderTimepoint0=[];
                            indxNotEmpty=[];
                            MouseNameTimepoint0=[];
                            InitialTimepointtxtFile=[];
                                timeFid=fopen(fullfile(TumourMaskAndStepsDir,'Timepoint.txt'),'r');
                                ImgTimepoint_temp=fscanf(timeFid,'%s', [1, inf]);
                                CurrentTimepoint=datetime(sprintf('%s %s',ImgTimepoint_temp(1:11),ImgTimepoint_temp(12:end)))%extracting precise date information of DAQ for determination of relative time elapsed automatically  old way:[~,Timepoint,]=fileparts(fileparts(MouseFolders{ind}));
                        end
                        %% 2D/3D coregistration, Gross tumour response metrics and longitudinal coregistration
                        % If this step was already performed it
                        % can be skipped
                        if AlreadyCompletedTumourMask2D==0 || AlreadyCompletedJustContouring==0 || AlreadyCompletedMaskCreation==0
                            toc
                            if LateralorFullCoregPreTisContour==0
                                if UnformatedDataSet==1 || filenameStOCT==0 %No stOCT data file
                                    BatchOfFolders = VOIorTumourMaskPrep_Metextraction_LongCoreg_fun17_NoStOCT(RepeatCheck2D,CurrentTimepoint,Timepoint0Analyzed,TumourMaskFrom_FLU_0_BRI_1_Varies_2,GlassThickness_200PixDepth,DataCroppedNotResizedInDepth,ReferencePixDepth,BatchOfFolders,countBatchFolder,MouseName,TimepointTrueAndRel,NameDayFormat,NameTimepointComboTemp,DirectoryInitialTimepoints,DirectoryVesselsData,DirectoryDataLetter,FolderConsidered,DirectoriesBareSkinMiceKeyword,OptFluSegmentationFolder,MaskCreationDraft,saveFolder,pathOCTVesRaw,filenameOCTVesRaw,pathOCTVesBin,filenameOCTVesBin,pathStOCT,filenameStOCT,RawVasculatureFileKeywordRaw,TryAutomaticAllignment,exposureTimes_BriFlu,num_contoured_slices,OSremoval,TumourMaskAndStepsDir,OCTLateralTimepointCoregistrationFolder,FoundTumourMask2DButCorrectAlignment,SaveFilenameDataUnlimited,SaveFilenameDataCylindricalProj,SaveFilenameDataUnlimited_UnCoregT0,SaveFilenameDataCylindricalProj_UnCoregT0,BinVesselsAlreadyPrepared,Mask2DandMetsOnly_0_OR_Mask3DOnlyAfter_1_OR_AllAtOnce_2,VisualizeResults,Neither_0_glass_1_Tissue_2_both_3_contour,DoseReceivedUpToTP,DaysPrecise,OnlyTissueLabelTimepoint0,ActiveMemoryOffload);
                                else
                                ...
                                if UnformatedDataSet==1
                                    ...
                                    BatchOfFolders = VOIorTumourMaskPrep_Metext_LongCoreg_fun16_CoregBefContTis(RepeatCheck2D,CurrentTimepoint,Timepoint0Analyzed,TumourMaskFrom_FLU_0_BRI_1_Varies_2,GlassThickness_200PixDepth,DataCroppedNotResizedInDepth,ReferencePixDepth,BatchOfFolders,countBatchFolder,MouseName,NameDayFormat,NameTimepointComboTemp,DirectoryInitialTimepoints,DirectoryVesselsData,DirectoryDataLetter,TimepointTrueAndRel,TimepointVarName,FolderConsidered,DirectoriesBareSkinMiceKeyword,OptFluSegmentationFolder,MaskCreationDraft,saveFolder,pathOCTVesRaw,filenameOCTVesRaw,pathOCTVesBin,filenameOCTVesBin,pathStOCT,filenameStOCT,RawVasculatureFileKeywordRaw,TryAutomaticAllignment,exposureTimes_BriFlu,IndexMouse,num_contoured_slices,OSremoval,TumourMaskAndStepsDir,OCTLateralTimepointCoregistrationFolder,FoundTumourMask2DButCorrectAlignment,SaveFilenameDataUnlimited,SaveFilenameDataCylindricalProj,SaveFilenameDataUnlimited_UnCoregT0,SaveFilenameDataCylindricalProj_UnCoregT0,BinVesselsAlreadyPrepared,Mask2DandMetsOnly_0_OR_Mask3DOnlyAfter_1_OR_AllAtOnce_2,VisualizeResults,Neither_0_glass_1_Tissue_2_both_3_contour,DoseReceivedUpToTP,DaysPrecise,SegmentationFolderTimepoint0,indxNotEmpty,MouseNameTimepoint0,OnlyTissueLabelTimepoint0,InitialTimepointtxtFile,ActiveMemoryOffload);
                                end
                            end
                            toc
                        end
    ...
    for ind=1:length(allMice)
        ...
        for trialOCTARaw=1:length(RawVasculatureFileKeywordRaw)
            if contains(allMice(ind).name,RawVasculatureFileKeywordRaw{trialOCTARaw}) && ~contains(allMice(ind).folder,'Old') && ~contains(allMice(ind).folder,'old') && ~contains(allMice(ind).folder,'Quick') && ~contains(allMice(ind).folder,'quick') && contains(allMice(ind).folder,'Improved') && ~contains(allMice(ind).folder,'TumourDepth') && ~contains(allMice(ind).folder,'Backside')%'Raw_OCT_Volume') % ~isempty(strfind(all(ind).name,'Raw_OCT_Volume'))
                ...
                %% Now checking if files already exist (alternatively skips)
                %if ~exist(savebotContour) && (Mask2DandMetsOnly_0_OR_Mask3DOnlyAfter_1_OR_AllAtOnce_2==1)% || Mask2DandMetsOnly_0_OR_Mask3DOnlyAfter_1_OR_AllAtOnce_2==2)
                MissingFilesForAutomation= Mask2DandMetsOnly_0_OR_Mask3DOnlyAfter_1_OR_AllAtOnce_2==1 && Neither_0_glass_1_Tissue_2_both_3_contour==0  ||(Mask2DandMetsOnly_0_OR_Mask3DOnlyAfter_1_OR_AllAtOnce_2==1) && ((Neither_0_glass_1_Tissue_2_both_3_contour==1 && ~exist(saveContour{1})) || (Neither_0_glass_1_Tissue_2_both_3_contour==2 && ~exist(saveContour{2})) || (Neither_0_glass_1_Tissue_2_both_3_contour==3 && ~exist(saveContour{1}) && ~exist(saveContour{2})))
                if MissingFilesForAutomation==1
                    fprintf('Missing 2D contours (tissue or glass mode according to setting), skipping\n')
                    continue
                else
                    AlreadyCompletedTumourMask2D=exist(TumourMask2DUncoregT0Filename,'file') && ~RepeatCheck2D%even if completed previously will go over check (but not actually necessarily repeated creation of mask just final steps of confirming mask is okay 
                    AlreadyCompletedJustContouring= ((Neither_0_glass_1_Tissue_2_both_3_contour==1 && exist(saveContour{1}) ) || (Neither_0_glass_1_Tissue_2_both_3_contour==2 && exist(saveContour{2})) || (Neither_0_glass_1_Tissue_2_both_3_contour==3 && exist(saveContour{1}) && exist(saveContour{2})))
                    ...
                    AlreadyCompletedBinarizationIfapplicable=BinarizeVesselsNOW_0_no_1_manual_2_automatic==0|| (FoundBinOCTA && (BinarizeVesselsNOW_0_no_1_manual_2_automatic==1 || BinarizeVesselsNOW_0_no_1_manual_2_automatic==2))
                    %% Final check to determine whether should procede with processing
                    if  (Neither_0_glass_1_Tissue_2_both_3_contour==0 && AlreadyCompletedTumourMask2D)||(AlreadyCompletedTumourMask2D && AlreadyCompletedJustContouring && Mask2DandMetsOnly_0_OR_Mask3DOnlyAfter_1_OR_AllAtOnce_2==0)|| (AlreadyCompletedTumourMask2D && AlreadyCompletedJustContouring && AlreadyCompletedMaskCreation && (Mask2DandMetsOnly_0_OR_Mask3DOnlyAfter_1_OR_AllAtOnce_2==1 || Mask2DandMetsOnly_0_OR_Mask3DOnlyAfter_1_OR_AllAtOnce_2==2))&& AlreadyCompletedBinarizationIfapplicable% && exist(fullfile(saveFolder,'CheckedOrientationsL12.txt'))%exist(fullfile(saveFolder,'CheckedOrientations.txt'))
                        fprintf('Already completed\n')
                        continue %skip this iteration of for loop and go to next iteration since already done previously
                        ...
                        if Timepoint0Analyzed==1 %Is the analysis being conducted before timepoint 0- has been acquired and been processed
                            ...
                            else%Day 0 pre is what is being processed now
                                if Neither_0_glass_1_Tissue_2_both_3_contour==1 || Neither_0_glass_1_Tissue_2_both_3_contour==3
                                    SegmentationFolderTimepoint0{1}=fullfile(fileparts(InitialTimepointtxtFile),'Manual_With_WO_Glass2');
                                end
                                if Neither_0_glass_1_Tissue_2_both_3_contour==2 || Neither_0_glass_1_Tissue_2_both_3_contour==3
                            ...
                            for ii=1:length(SegmentationFolderTimepoint0)
                                if ~isempty(SegmentationFolderTimepoint0{ii})
                                    indxNotEmpty=ii %which segmentation types complete if I recall (tissue or glass)
                                    break;
                                end
                            end
                        ...
                        if FoundBinOCTA==1
                            ...
                            if FoundstOCT==1
                                ...
                                if FoundMask2D==0
                                    if PredrawnTumourMaskMaybeMisnamed==1 %if previously created mask will look for it
                                        [filename2DMask,path2DMask]=uigetfile(OptFluSegmentationFolder,'Please select previously created tumour mask file (if none just cancel).')
                                        if isequal(filename2DMask,0) || isequal(path2DMask,0)%not found this previously drawn mask
                                            createTumourMaskQuestion=input('Shall we create one and extract all the metrics (fast,<8min)? y/n \n','s')
                                            if ~isequal(createTumourMaskQuestion,'y')|| isempty(createTumourMaskQuestion)
                                        ...
                                        filename2DMask=0;%Not found and user indicated that they did not previously process it so no need to search for it.
                                    end
                                end
                                if isequal(filename2DMask,[PrefixFLU_BRI,'TumourMask2D_aligned_coregTime0.mat']) %Selecting this only means it skips even checking for alignment step (between TPX OCT and TPX opt flu
                                    FoundTumourMask2DButCorrectAlignment=1; %if it is named this way, it was created with the script for aligning
                                ...
                                elseif isequal(filename2DMask,0)
                                    createTumourMaskQuestion='y';
                                    FoundTumourMask2DButCorrectAlignment=0;
                                else%if isequal(filename2DMask,[PrefixFLU_BRI,'TumourMask2D_aligned.mat'])
                                    createTumourMaskQuestion='y';
                                    FoundTumourMask2DButCorrectAlignment=2;%some other mask intermediate
                                end
                            end
